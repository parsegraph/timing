...
app.get("/", async (req, res) => {
  ...
  write(`<html>`);
  write(`<head>`);
  write(`<title>timing</title>`);
  write(`</head>`);
  write(`<body>`);
  write(
    `<h1>timing <a href='/coverage'>Coverage</a> <a href='/docs'>Docs</a></h1>`
  );
  write(
    `<p>This library is available as JavaScript UMD module: <a href='/parsegraph-timing.js'>parsegraph-timing.js</a></p>`
  );
...
app.listen(port, () => {
  console.log(`See timing build information at http://localhost:${port}`);
});
