import { IntervalTimer } from "./index";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  container.innerHTML = `Parsegraph Timing`;
  ...
  const dotTimer: IntervalTimer = new IntervalTimer();
  dotTimer.setDelay(500);
  dotTimer.setListener(refreshDot);
  const timer: IntervalTimer = new IntervalTimer();
  timer.setListener(refresh);
  timer.setDelay(3000);
